...
def parser_xml_detail(self_doc, xml_doc):
    ...
    tax_rate_obj = self_doc.env['xtax.rate'].search([])

    # tag_DetalleServicio = getElementTag(doc, 'DetalleServicio')
    if getElementTag(doc, 'DetalleServicio'):
        tag_LineaDetalle = doc.getElementsByTagName('LineaDetalle')
        for detail_line in tag_LineaDetalle:
            tag_CodigoComercial = getElementTag(detail_line, 'CodigoComercial')     # solo toma 1 tag codigoComercial
            comercial_code =  getElementTag_data(tag_CodigoComercial.getElementsByTagName('Codigo')) if tag_CodigoComercial else None
            tag_Impuesto = detail_line.getElementsByTagName('Impuesto')
            tax_vals = []
            # num = []
            for tax_line in tag_Impuesto:
                # num.append(getElementTag_data(tax_line.getElementsByTagName('Monto')))
                codigo = getElementTag_data(tax_line.getElementsByTagName('Codigo'))
                codigo_tarifa = getElementTag_data(tax_line.getElementsByTagName('CodigoTarifa'))
                tax_code = tax_code_obj.filtered(lambda r: r.code == codigo and r.version == self_doc.version)
                rate_code = tax_rate_obj.filtered(lambda r: r.code == codigo_tarifa)
                tax_elem = {
                            'code': codigo,
                            'code_id': tax_code.id,
                            'rate_code': codigo_tarifa,
                            'rate_code_id': rate_code.id,
                            'rate': getElementTag_data(tax_line.getElementsByTagName('Tarifa')),
            ...
            montoTotalLinea = float(getElementTag_data(detail_line.getElementsByTagName('MontoTotalLinea')))
            montoTotal = float(getElementTag_data(detail_line.getElementsByTagName('MontoTotal')))
            values = {
                'document_id': self_doc.id,
                'line_number': getElementTag_data(detail_line.getElementsByTagName('NumeroLinea')),
                'cabys_code': getElementTag_data(detail_line.getElementsByTagName('Codigo')),
                'comercial_code': comercial_code,
                'quantity': getElementTag_data(detail_line.getElementsByTagName('Cantidad')),
                'description': getElementTag_data(detail_line.getElementsByTagName('Detalle')),
                'measurement_unit': getElementTag_data(detail_line.getElementsByTagName('UnidadMedida')),
                'price_unit': getElementTag_data(detail_line.getElementsByTagName('PrecioUnitario')),
