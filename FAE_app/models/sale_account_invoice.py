...
class FaeAccountInvoice(models.Model):
    ...
    x_fae_incoming_doc_id = fields.Many2one("xfae.incoming.documents", string="Doc.Electrónico",
                                        required=False,
                                        domain="[('document_type','=',x_document_type),('issuer_identification_num','=',x_partner_vat),"
                                               "('company_id','=',company_id),('ready2accounting','=',True),('invoice_id','=',False)]")
    ...
    def _onchange_partner_id(self):
        ...
        if not self.x_enable_no_document_type:
            ...
            if self.move_type == 'out_refund':
                ...
                if self.partner_id and self.partner_id.x_foreign_partner:
    ...
    def _document_type_changed(self):
        ...
        elif self.x_document_type in ('FE', 'NC') and self.partner_id and (not self.partner_id.x_identification_type_id or not self.partner_id.vat):
    ...
    def action_post(self):
        ...
        for inv in self:
            if not inv.is_invoice() or inv.x_state_dgt or (inv.x_enable_no_document_type and not inv.x_document_type) \
                    or (inv.is_purchase_document() and not inv.x_document_type and inv.partner_id.x_foreign_partner):
                inv.compute_name_value()
                super(FaeAccountInvoice, inv).action_post()
                continue
            # is_invoice
            elif inv.is_invoice() and not inv.x_document_type and not inv.x_enable_no_document_type:
    ...
    def generate_xml_and_send_dgt(self, invoices, write_log=False):
        ...
        for inv in invoices:
            try:
                count_inv += 1
                if write_log:
                    _logger.info('>> generate_xml_and_send_dgt:  - fae_mode: %s,  identif_type: %s, move_type: %s,  sequence: %s'
                ...
                if write_log:
                    _logger.info('>> generate_xml_and_send_dgt: numero: %s / %s  -  consecutivo: %s', count_inv, quantity_invoices, inv.x_sequence)
                        ...
                        if sequence.number_next_actual >= 5:
                            ...
                            sql_cmd = """
                                ...
                                """.format(from_seq=from_x_sequence, to_seq=prev_x_sequence, pref_seq=from_x_sequence[:10]
                                           , doc_type=inv.x_document_type, company_id=inv.company_id.id)
                            if fae_utiles.modulo_instalado(self.env, 'point_of_sale'):
                                sql_cmd = sql_cmd + """
                                UNION
                                SELECT x_sequence
                                FROM pos_order
                ...
                _logger.error('>> generate_xml_and_send_dgt.exception, Error: %s', str(error))  # no se registra en el Log, porque el raise lo hace
                msg_body = 'generate_xml_and_send_dgt.exception:  Aviso!.\n Error: %s' % str(error)
    ...
    def create(self, vals_list):
        # OVERRIDE
        is_incoming_doc = False
        for vals in vals_list:
            if 'x_fae_incoming_doc_id' in vals and vals.get('x_fae_incoming_doc_id'):
                is_incoming_doc = True
        if is_incoming_doc:
            res = super(FaeAccountInvoice, self.with_context(check_move_validity=False)).create(vals_list)
            incoming_doc = self.env['xfae.incoming.documents'].search(
                        [('id', '=', int(vals.get('x_fae_incoming_doc_id')))], limit=1)
            incoming_doc.write({'invoice_id': res.id, 'purchase_registried': True})
            return res
        return super(FaeAccountInvoice, self).create(vals_list)

    # carga lineas del XML para facturas de Compra
    # @api.onchange('x_fae_incoming_doc_id', 'journal_id')
    def action_load_lines_incoming_doc(self):
        return
        # no se ejecuta hasta que este liberado    
        for move in self:
            if move.x_fae_incoming_doc_id and move.journal_id:
                if (not move.is_purchase_document() or not move.x_fae_incoming_doc_id or not move.x_fae_incoming_doc_id.issuer_xml_doc
                    or move.x_document_type not in ('NC','FE')):
                    return
                if move.currency_id != move.x_fae_incoming_doc_id.currency_id:
                    raise UserError("La moneda del documento recibido es: %s" % move.x_fae_incoming_doc_id.currency_id.name)
                if move.x_fae_incoming_doc_id.lines_count == 0:
                    if not move.x_fae_incoming_doc_id.carga_lineas_xml():
                        raise ValidationError("Al documento recibido no se le pudo extraer la líneas de detalle del XML")

                move.invoice_line_ids = [[6, 0, []]]
                move_lines = move.x_fae_incoming_doc_id.get_purchase_move_lines(self)

                move.invoice_line_ids = move_lines
